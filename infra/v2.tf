...
resource "azurerm_resource_group" "rg" {
  name     = "my3tierapp-rg-multi-vm-21212"
  location = "UK South"
}
...
resource "azurerm_application_gateway" "appgw" {
  ...
  # ✅ Add this block to fix TLS issue
  ssl_policy {
...
resource "azurerm_key_vault" "kv" {
  name                       = "my-app-kv-${random_string.kv_suffix.result}"
  location                   = azurerm_resource_group.rg.location
  resource_group_name        = azurerm_resource_group.rg.name
  sku_name                   = "standard"
...
resource "azurerm_key_vault_secret" "app_secret" {
  ...
  depends_on   = [azurerm_mysql_flexible_server.db, azurerm_key_vault.kv]
}

# Role assignment for VMSS to read secrets
...
# Role assignment for Terraform user to manage secrets
...
# App setup
APP_DIR="/home/azureuser/azure-3tier-app"
sudo -u azureuser mkdir -p $APP_DIR
cd $APP_DIR

# Clone your repo
sudo -u azureuser git clone https://github.com/ShamailAbbas/azure-3tier-app . || true
...
# Install app dependencies as azureuser
sudo -u azureuser bash -c "cd $APP_DIR/backend && npm install"

# Start the app with PM2 as azureuser
sudo -u azureuser bash -c "cd $APP_DIR/backend && pm2 start index.js --name myapp"

# Save PM2 process list
sudo -u azureuser pm2 save

# Configure PM2 startup for azureuser
...
output "mysql_server_fqdn" {
  ...
  description = "FQDN of MySQL Flexible Server"
  sensitive   = true
}
